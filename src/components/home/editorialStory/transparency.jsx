...
const formulaImages = [
  {
    src: "/images/urunler-ham/glutanex-glow-therapy-ampoule-hover.png",
    alt: "GLUTANEX Glow Therapy Ampoule ürün ve doku görünümü",
    className: "min-[1024px]:mt-28",
    distance: -72,
  },
  {
    src: "/images/urunler-ham/glutanex-aqua-booster-hover.png",
    alt: "GLUTANEX Aqua Booster ürün ve doku görünümü",
    ...
    >
      <div className="relative z-2" data-motion-group>
        <div className="grid items-end gap-8 min-[1024px]:grid-cols-12">
          ...
          <div
            className="max-w-[420px] min-[1024px]:col-span-4 min-[1024px]:justify-self-end min-[1024px]:pb-3"
            data-section-reveal
          >
            <p className="text-[clamp(1rem,1.25vw,1.15rem)] leading-[1.5] tracking-[-0.025em] text-site-copy">
              Üründe öne çıkan aktifleri, dokuyu ve kullanım adımını açıkça
              anlatıyoruz. Seçimini belirsiz vaatlerle değil, anlaşılır bilgiyle
              kurmanı istiyoruz.
            </p>
            <a
              className="group mt-7 inline-flex items-center gap-3 text-[0.7rem] font-semibold tracking-[0.09em] text-site-ink uppercase"
              href="#urunler"
            >
              Ürünleri incele
        ...
        <div className="mt-16 grid gap-10 min-[768px]:mt-24 min-[1024px]:grid-cols-12 min-[1024px]:gap-[clamp(2rem,5vw,6.5rem)]">
          <div
            className="grid grid-cols-2 items-center gap-4 min-[1024px]:col-span-6 min-[1024px]:gap-6"
            data-scroll-parallax-section
            data-parallax-strength="1.05"
          >
            {formulaImages.map((image) => (
              <figure
                className={`relative aspect-[4/5] overflow-hidden rounded-[48%_52%_44%_56%/34%_33%_67%_66%] bg-white/90 ${image.className}`}
                key={image.src}
                data-scroll-parallax-layer
                data-parallax-distance={image.distance}
              >
                <Image
          ...
          <div className="min-[1024px]:col-span-6 min-[1024px]:pt-24">
            <div className="grid gap-x-8 border-t border-site-ink/20 min-[640px]:grid-cols-2">
              {ingredientStories.map((ingredient) => (
                <article
                  className="border-b border-site-ink/20 py-7 min-[640px]:py-9"
                  key={ingredient.index}
                  data-section-reveal
                >
                  <span className="text-[0.62rem] font-semibold tracking-[0.13em] text-site-copy uppercase">
                    {ingredient.index}
                  </span>
                  <h3 className="mt-4 text-[clamp(1.35rem,2.2vw,2rem)] leading-[1] font-light tracking-[-0.045em] text-site-ink">
                    {ingredient.name}
                  </h3>
                  <p className="mt-4 max-w-[34ch] text-[0.88rem] leading-[1.55] text-site-copy">
